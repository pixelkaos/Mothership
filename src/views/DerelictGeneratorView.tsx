...
import { DERELICT_GENERATION_TABLE, CAUSE_OF_RUINATION, WEIRD, CARGO_TYPE, SHIP_NAME_PART1, SHIP_NAME_PART2, SHIP_NAME_PART3 } from '@/constants/derelict';
...
export const DerelictGeneratorView: React.FC = () => {
    ...
    const handleGenerateShip = useCallback(() => {
        ...
        const roll = rollDice('1d100', { zeroBased: true }); // 0–99 for table lookup
        ...
        // rollDice('1d100') returns 0–99; use directly for table indexing.
        const cause = CAUSE_OF_RUINATION[rollDice('1d100', { zeroBased: true })];
        const weird = WEIRD[rollDice('1d100', { zeroBased: true })];
        const cargo = CARGO_TYPE[rollDice('1d100', { zeroBased: true })];
    ...
    const handleEnhanceWithAI = useCallback(async () => {
        ...
        try {
            ...
            console.error(e);
            const msg = e?.message ? `AI error: ${e.message}` : 'Failed to get a response from the AI. The signal might be lost...';
            setError(msg);
        } finally {
            setIsLoading(false);
        }
