...
export const Card: React.FC<CardProps> = ({ title, description, visualContent, action, onClick, className }) => (
    <div
        className={`group transition-all duration-2 hover:-translate-y-1 ${onClick ? 'cursor-pointer' : ''} ${className}`}
        onClick={onClick}
    >
        <Panel 
            tone="default" 
            className="flex flex-col h-full text-left group-hover:bg-black/20 group-hover:border-muted/40 group-hover:shadow-elev-1"
        >
            {visualContent && (
